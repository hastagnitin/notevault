...
function Dashboard() {
  ...
  const renderMainContent = () => {
    switch (activeView) {
      ...
      case 'ai':
        return (
          <div className="book-container flex w-full gap-6 lg:gap-8 flex-col lg:flex-row" ref={bookRef}>
            {/* LEFT PAGE - Notes / Quick Actions */}
            <div className="page w-full lg:w-[40%] flex flex-col gap-6" ref={el => pagesRef.current[0] = el}>
            ...
            {/* RIGHT PAGE - AI Chat Assistants */}
            <div className="page w-full lg:w-[60%] flex flex-col" ref={el => pagesRef.current[1] = el}>
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden font-sans relative z-0" style={{ backgroundColor: '#050714', color: '#FFFFFF' }}>
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative z-10" style={{ backgroundColor: '#050714' }}>
        <div className="z-30 animate-fade-in-up flex-shrink-0" style={{ animationFillMode: 'both', animationDuration: '0.4s' }}>
          <TopNavigation />
        </div>

        <main className="flex-1 flex flex-col p-6 lg:p-8 gap-6 overflow-x-hidden overflow-y-auto w-full max-w-[1400px] mx-auto perspective-1000">
          {renderMainContent()}
        </main>
